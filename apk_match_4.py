...
def load(app_to_classify):
    ...
    if len(test_bow):
        ...
        if pair[1] >= 0.7:
            ...
            data_init.close()
            data = open("D:\\work\\apk\\xenderDemo.txt", 'r', encoding='utf8')
...
def main():
    infile_init = open("D:\\work\\apk\\xenderCleanAll.txt", 'r', encoding='utf8')
    words_init = []
    # 加载数据
    line_num = 0
    for line in infile_init:
        line_num += 1
        if 51001 <= line_num <= 68000:
            # print(line[0:-1])
...
if __name__ == '__main__':
    ...
    print(time_begin)
    with open("D:\\work\\apk\\timeCountAllDot7.txt", "a", encoding='utf8') as f:
        f.write("time_begin_4:" + "{}".format(time_begin) + '\n')
    logging.basicConfig(
    ...
    print(time_end)
    with open("D:\\work\\apk\\timeCountAllDot7.txt", "a", encoding='utf8') as f:
        f.write("time_end_4" + "{}".format(time_end) + '\n')
    print(time.clock())
